...
5. Clean up the **Song List** — normalize leader names, assign each non-default book a SingLoud code, tag Memorial/Closing/Call Back rows as needed. If the file has more than one session, review the Sessions section that appears here.
...
- **Tunebook Index tab** — the full page-by-page index of any tunebook this suite has data for, browsable and searchable (one search box matches against both page number and title together) — a quick reference for double-checking a page or title while entering songs, independent of the current project's own song list.
...
| `Order of entry` | Not required — rows are re-numbered on import. |
...
- **SHMHA's own codes** (published in the *Minutes Book*, page 44) are used *only* in the dedicated SHMHA export, translated automatically from the SingLoud code you assign.
